...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        data = data.to(device)
        optimizer.zero_grad()
        output,gat_att_weights,egat_edge_indx = model(data,inp)
...
def predicting(model, device, loader):
    ...
    with torch.no_grad():
        for data in loader:
            data = data.to(device)
            output,gat_att_weights,gat_edge_indx= model(data,inp)
            total_preds =  output.cpu()
            total_labels = data.y
    return total_labels,total_preds,gat_att_weights,gat_edge_indx#.numpy(),total_preds.numpy()#.flatten(),total_preds.numpy().flatten()
...
inp_1 = pickle.load(open('glove_wordEmbedding.pkl','rb'))
...
if ((not os.path.isfile(processed_train)) or (not os.path.isfile(processed_test))):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        G, P,M,W= predicting(model, device, test_loader)
        print(W.T)
        ...
        ret = [acc,precision,recall,ham_l,coverage,one_error,RL]
        if acc > max_acc:
            max_acc = acc
